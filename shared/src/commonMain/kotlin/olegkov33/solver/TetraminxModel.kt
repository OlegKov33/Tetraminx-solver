    ...
    fun createFace(faceNum: Int) {
        ...
        return Canvas(modifier = Modifier
                ...
                detectTapGestures{pos ->
                    pathList.forEachIndexed { index, path->
                        ...
                        if(PathHitTester(path).contains(pos)){
                            println("what is $path and $pos?")

                            println("pressed $index on face $faceNum")
                        }
                    }
                }
            }
            ...
            for( i in pathList.indices){
                if(i % 2 == 0){
                    drawPath(pathList[i], Color.Black)
                }
                else{
                    drawPath(pathList[i], Color.LightGray)
                }
            }
